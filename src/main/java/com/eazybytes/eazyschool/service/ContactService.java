...
import com.eazybytes.eazyschool.constants.EazySchoolConstant;
...
public class ContactService {
    ...
    public Page<Contact> findMsgsWithOpenStatus(int pageNum,String sortField,String sortDir){
        ...
        Pageable pageable = PageRequest.of(pageNum -1 ,pageSize,sortDir
                .equals("asc") ? Sort.by(sortField).ascending()
                : Sort.by(sortField).descending());

        Page<Contact> msgPage = contactRepository.findByStatus(EazySchoolConstant.OPEN,pageable);
    ...
    public boolean updateMsgStatus(int contactId){
        ...
        int rows = contactRepository.updateMsgStatusNative(EazySchoolConstant.CLOSE,contactId);
        if (rows > 0){
            isUpdated = true;
        }
        return isUpdated;
